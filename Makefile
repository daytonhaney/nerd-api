...
volume:
		...
		docker volume inspect py-api_local_postgres_data

nerds-db:

		docker compose -f local.yml exec postgres psql --username=nerd1 --dbname=nerd-db 
...
black-check:
		...
		docker comopse -f local.yml exec api black --check --exclude=migrations . 
